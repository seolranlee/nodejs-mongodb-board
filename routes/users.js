...
function checkUserRegValidation(req, res, next){
    ...
    async.waterfall(
        [function (callback) {
            Users.findOne({email: req.body.user.email, id: {$ne: mongoose.Types.ObjectId(req.params.id)}},
                function (err, user) {
                    if(user){
                        isValid = false;
            ...
            Users.findOne({nickname: req.body.user.nickname, _id: {$ne: mongoose.Types.ObjectId(req.params.id)}},
                function (err, user) {
                    if(user){
                        isValid=false;
